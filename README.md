...
[![Tests](https://img.shields.io/badge/Tests-73%20passing-22c55e?style=flat-square&logo=pytest&logoColor=white)](tests/)
...
---
> **rika-agent** is a privacy first open source agentic system that you can control via Telegram (using a bot), via a REST API, or programmatically as a Python library.
  
>v2.2 ships JSON-native function calling, provider failover with live countdown UX, a three-level code sandbox, background system monitoring, vision input, file delivery, encrypted memory, and support for Groq / Gemini / OpenRouter / NVIDIA NIM / Vercel AI / Ollama / G4F — running entirely on your own server, with your data staying yours.

> **Multi-platform:** Telegram polling, REST API (Flask), Discord (stub), and library mode — all share the same core infrastructure via `PlatformRouter` and `EventSink`. Enable any combination via `.env` variables.
...
**Requirements:** Python 3.12+, Telegram bot token from [@BotFather](https://t.me/BotFather) (for Telegram mode) or just Python for API-only mode.
...
python3 -m pytest tests/ --ignore=tests/test_crypto.py -v
```
...
# API mode (optional)
# API_HOST=0.0.0.0   # uncomment to enable REST API on port 5000
# API_PORT=5000
# API_DEBUG=false
...
API (Flask server)
├── POST /api/chat ────► run_orchestration_background()
│     X-API-Key auth         → buffered responses returned as JSON
└── GET /api/health    ► {"status": "ok"}

Discord (placeholder)
└── DiscordAdapter — stub, not yet implemented

EventSink
└── PlatformRouter
    ├── "tg:"      → TelegramAdapter
    ├── "api:"     → APIAdapter
    └── "discord:" → DiscordAdapter (stub)
...
│   ├── interfaces/
│   │   ├── telegram/            Telegram adapter + work log
│   │   ├── api/                 REST API adapter + Flask server
│   │   └── discord/             Discord adapter (stub — needs discord.py)
│   ├── core/
│   │   ├── orchestrator.py     main orchestration loop
│   │   ├── complexity.py       3-tier message classifier
│   │   ├── context.py          per-turn context assembler
│   │   ├── event_bus.py        SessionEvent pub/sub
│   │   ├── event_sink.py       multi-platform delivery via PlatformRouter
│   │   ├── router.py           PlatformRouter: prefix-based adapter dispatch
...
├── tests/
│   ├── test_config.py          Config loading, platform detection, cache
│   ├── test_event_sink.py      EventSink single/multi-adapter routing
│   ├── test_orchestrator.py    Orchestrator init, notify, provider failover, run loop
│   ├── test_parse_keys.py      Key text parsing
│   ├── test_pg_compat.py       SQLite→Postgres SQL translation (17 tests)
│   ├── test_provider_pool.py   Key failover, auth, transient retry
│   └── test_router.py          PlatformRouter prefix routing, fallback
├── scripts/
