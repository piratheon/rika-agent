...
_buses: dict[str, asyncio.Queue[SessionEvent]] = defaultdict(
    lambda: asyncio.Queue(maxsize=200)
)

_fanout: dict[str, list[asyncio.Queue[SessionEvent]]] = defaultdict(list)


def get_bus(session_id: str) -> asyncio.Queue[SessionEvent]:
    """Return the primary event queue for a session."""
    return _buses[session_id]


async def emit(session_id: str, event: SessionEvent) -> None:
...
def subscribe(session_id: str) -> asyncio.Queue[SessionEvent]:
...
def unsubscribe(session_id: str, q: asyncio.Queue[SessionEvent]) -> None:
...
async def drain(session_id: str, timeout: float = 0.1) -> list[SessionEvent]:
...
def teardown(session_id: str) -> None:
